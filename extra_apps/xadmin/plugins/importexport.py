...
class ImportView(ImportBaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        uploaded files to a local temp files that will be used by
        ...
        if request.POST and form.is_valid():
            ...
            # first always write the uploaded files to disk as it may be a
            # memory files or else based on settings upload handlers
            ...
            # then read the files, using the proper format-specific mode
            ...
            try:
                ...
                return HttpResponse(_(u"<h1>Imported files has a wrong encoding: %s</h1>" % e))
            except Exception as e:
                return HttpResponse(_(u"<h1>%s encountered while trying to read files: %s</h1>" % (type(e).__name__,
                                                                                                  import_file.name)))
